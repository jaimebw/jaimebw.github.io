...
## Introduction
The brew experiment
For the last couple of weeks my laptop has been having some issues regarding performance. Therefore, I was thinking of formatting my SSD in order to see if that would help regaining its old benchmark. The problem of this process has to do with having to reinstall all the software again which is a really boring process and I wanted to make it in a more automatic way.  
Lucky for me I have [Homebrew](https://brew.sh/)installed and I know a little bit of **zsh** scripting and Python so I started playing with those two tools.  
## Experiment
My main objective was to create a .py script that could generate a .zsh script from a .txt with the software I previously had installed. So, the first thing to do is to get the software installed in by Homebrew. I run on the Terminal the command:
```
brew list
```
